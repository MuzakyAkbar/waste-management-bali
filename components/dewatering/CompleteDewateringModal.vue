      ...
      <div class="inline-block align-bottom bg-white rounded-xl text-left overflow-hidden shadow-xl transform transition-all sm:my-8 sm:align-middle sm:max-w-2xl sm:w-full">
        ...
        <div class="bg-white px-4 pt-5 pb-4 sm:p-6 border-b border-gray-200">
          <div class="flex items-center justify-between">
            <div>
              <h3 class="text-lg leading-6 font-bold text-gray-900">Selesaikan Dewatering</h3>
        ...
        <div class="px-4 py-5 sm:p-6 space-y-6 max-h-[70vh] overflow-y-auto">
          ...
          <div class="bg-cyan-50 rounded-lg p-4 border border-cyan-100">
            <h4 class="text-xs font-bold text-cyan-800 uppercase tracking-wider mb-3">Data Awal (Referensi)</h4>
            <div class="grid grid-cols-2 gap-4 text-sm">
              <div>
                <p class="text-gray-500 text-xs">Waktu Mulai</p>
                <p class="font-medium">{{ formatDate(process.activity_date) }}</p>
              </div>
              <div>
                <p class="text-gray-500 text-xs">KWh Awal</p>
          ...
          <div class="grid grid-cols-1 sm:grid-cols-2 gap-5">
            <div class="sm:col-span-2 grid grid-cols-2 gap-4">
              <div>
                ...
                <input type="date" v-model="form.end_date" class="block w-full border-gray-300 rounded-lg p-2.5 border focus:ring-cyan-500 focus:border-cyan-500">
              </div>
              <div>
                <label class="block text-sm font-medium text-gray-700 mb-1">Jam Selesai</label>
                <input type="time" v-model="form.end_time" class="block w-full border-gray-300 rounded-lg p-2.5 border focus:ring-cyan-500 focus:border-cyan-500">
              </div>
            </div>

            <div>
              <label class="block text-sm font-medium text-gray-700 mb-1">KWh Meter Akhir</label>
              <input type="number" step="0.1" v-model="form.kwh_end" class="block w-full border-gray-300 rounded-lg p-2.5 border focus:ring-cyan-500 focus:border-cyan-500" placeholder="0.0">
            ...
            <div>
              ...
              <input type="number" step="0.01" v-model="form.output_amount" class="block w-full border-gray-300 rounded-lg p-2.5 border focus:ring-cyan-500 focus:border-cyan-500" placeholder="0.00">
            </div>
          </div>
          ...
          <div>
            ...
            <textarea
              v-model="form.notes"
              rows="2"
              class="block w-full border-gray-300 rounded-lg p-2.5 border resize-none focus:ring-cyan-500 focus:border-cyan-500"
        ...
        <div class="bg-gray-50 px-4 py-3 sm:px-6 sm:flex sm:flex-row-reverse border-t border-gray-200">
          <button
            type="button"
            class="w-full inline-flex justify-center rounded-lg bg-cyan-600 px-4 py-2 text-white hover:bg-cyan-700 sm:ml-3 sm:w-auto items-center disabled:opacity-50 disabled:cursor-not-allowed"
            :disabled="loading"
            @click="handleSubmit"
          >
          ...
          <button
            type="button"
            class="mt-3 w-full inline-flex justify-center rounded-lg border border-gray-300 px-4 py-2 bg-white text-gray-700 hover:bg-gray-50 sm:mt-0 sm:ml-3 sm:w-auto"
            @click="closeModal"
            :disabled="loading"
          >
            Batal
...
const handleSubmit = async () => {
  ...
  try {
    ...
    for (const file of kwhEndFiles.value) {
      const uploadRes = await dewateringStore.uploadImage(file, 'dw-kwh-end-images', props.process.id)
      if (uploadRes.success) {
        kwhEndImagesArray.push({ url: uploadRes.url, path: uploadRes.path, bucket: uploadRes.bucket })
      }
    }

    const outputImagesArray = []
    for (const file of outputFiles.value) {
      const uploadRes = await dewateringStore.uploadImage(file, 'dw-output-images', props.process.id)
      if (uploadRes.success) {
        outputImagesArray.push({ url: uploadRes.url, path: uploadRes.path, bucket: uploadRes.bucket })
      }
    }
    ...
    const result = await dewateringStore.completeProcess(props.process.id, completionData)

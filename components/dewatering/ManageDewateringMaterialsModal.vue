  ...
  <div v-if="show" class="fixed inset-0 z-50 overflow-y-auto" aria-labelledby="modal-title" role="dialog" aria-modal="true">
    <div class="flex items-end justify-center min-h-screen pt-4 px-4 pb-20 text-center sm:block sm:p-0">
      ...
      <div class="inline-block align-bottom bg-white rounded-lg text-left overflow-hidden shadow-xl transform transition-all sm:my-8 sm:align-middle sm:max-w-6xl sm:w-full">
          ...
          <div class="sm:flex sm:items-start justify-between">
            <div class="mt-3 text-center sm:mt-0 sm:text-left w-full">
              <h3 class="text-lg leading-6 font-bold text-gray-900" id="modal-title">
                Kelola Material Input - Dewatering
              </h3>
              <p class="mt-1 text-sm text-gray-500">
                Tambahkan material dan foto bukti.
              </p>
            </div>
          ...
          <div v-else class="space-y-5">
            ...
            <div class="flex flex-col sm:flex-row justify-between items-center gap-4 border-b border-gray-100 pb-4">
              ...
              <button
                type="button"
                @click="addRow"
                class="w-full sm:w-auto inline-flex justify-center items-center px-4 py-2 border border-transparent text-sm font-medium rounded-md text-white bg-cyan-600 hover:bg-cyan-700 focus:outline-none shadow-sm"
                :disabled="loading"
              >
                Tambah Material
              </button>
              ...
              <table class="min-w-full divide-y divide-gray-200">
                ...
                <tbody class="bg-white divide-y divide-gray-200">
                  <tr v-for="(row, index) in rows" :key="row._id">
                    ...
                    <td class="px-4 py-4 whitespace-nowrap align-top">
                      <select v-model="row.material_id" class="block w-full border-gray-300 rounded-lg shadow-sm focus:ring-cyan-500 focus:border-cyan-500 sm:text-sm">
                        <option value="">-- Pilih Material --</option>
                        <option v-for="mat in availableMaterials" :key="mat.material_id" :value="mat.material_id">{{ mat.material_name }}</option>
                      </select>
                    </td>
                    <td class="px-4 py-4 whitespace-nowrap align-top">
                      <input type="number" min="1" v-model.number="row.container_number" class="block w-full border-gray-300 rounded-lg shadow-sm focus:ring-cyan-500 focus:border-cyan-500 sm:text-sm text-center">
                    </td>
                    <td class="px-4 py-4 align-top">
                      <input type="text" v-model="row.container_content" placeholder="Cth: Kasar, Basah" class="block w-full border-gray-300 rounded-lg shadow-sm focus:ring-cyan-500 focus:border-cyan-500 sm:text-sm">
                    </td>
                    <td class="px-4 py-4 align-top">
                      <button
                        ...
                        :class="row.imageUrl ? 'border-green-300 bg-white ring-2 ring-green-100' : 'bg-gray-50 border-gray-300 hover:border-cyan-400 hover:text-cyan-500 text-gray-400'"
                    ...
                    <td class="px-4 py-4 whitespace-nowrap align-top">
                      <div class="relative">
                        <input type="number" step="0.01" v-model.number="row.qty" class="block w-full border-gray-300 rounded-lg shadow-sm focus:ring-cyan-500 focus:border-cyan-500 sm:text-sm" placeholder="0.00">
                ...
                <tfoot class="bg-gray-50">
                  <tr>
                    ...
                    <td class="px-4 py-3 text-left text-sm font-bold text-cyan-700">{{ totalWeight }} kg</td>
                    <td></td>
                  </tr>
                </tfoot>
              </table>
            ...
            <div class="md:hidden space-y-4">
              <div v-for="(row, index) in rows" :key="row._id" class="bg-white border border-gray-200 rounded-lg p-4 shadow-sm">
                <div class="flex justify-between items-center mb-3">
                  <span class="inline-flex items-center justify-center w-6 h-6 rounded-full bg-cyan-100 text-cyan-800 text-xs font-bold">#{{ index + 1 }}</span>
                ...
                <div class="space-y-3">
                  ...
                  <div>
                    ...
                    <input type="text" v-model="row.container_content" placeholder="Deskripsi tambahan (opsional)" class="block w-full border-gray-300 rounded-lg text-sm">
                  </div>
              ...
              <div class="bg-cyan-50 border border-cyan-200 rounded-lg p-4 flex justify-between items-center">
                <span class="font-bold text-gray-700">Total Input:</span>
                <span class="font-bold text-cyan-700 text-lg">{{ totalWeight }} Kg</span>
              </div>
            </div>
        ...
        <div class="bg-gray-50 px-4 py-3 sm:px-6 sm:flex sm:flex-row-reverse border-t border-gray-200">
          <button
            type="button"
            class="w-full inline-flex justify-center rounded-lg border border-transparent shadow-sm px-4 py-2 bg-cyan-600 text-base font-medium text-white hover:bg-cyan-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-cyan-500 sm:ml-3 sm:w-auto sm:text-sm disabled:opacity-50 disabled:cursor-not-allowed items-center"
          ...
          <button
            type="button"
            class="mt-3 w-full inline-flex justify-center rounded-lg border border-gray-300 shadow-sm px-4 py-2 bg-white text-base font-medium text-gray-700 hover:bg-gray-50 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-cyan-500 sm:mt-0 sm:ml-3 sm:w-auto sm:text-sm"
            @click="closeModal"
            :disabled="loading"
          >
            Batal
    ...
    <div v-if="showSelectionModal" class="fixed inset-0 z-[60] flex items-center justify-center p-4 bg-black/70 backdrop-blur-sm" @click.self="showSelectionModal = false">
      <div class="bg-white w-full max-w-sm rounded-2xl p-6 shadow-2xl relative animate-fade-in">
        ...
        <div class="space-y-3">
          <button @click="openCamera" class="w-full flex items-center justify-center gap-3 bg-cyan-600 text-white py-3.5 rounded-xl font-semibold hover:bg-cyan-700 transition shadow-md">
...
const loadData = async () => {
  ...
  try {
    ...
    const { data: usedData, error: usedError } = await supabase
      ...
      .eq('dewatering_id', props.processId)
...
const handleFileUpload = async (file) => {
  if (!file || activeRowIndex.value === null) return
  ...
  try {
    ...
    const bucket = 'dw-material-images'
    const ext = file.name.split('.').pop() || 'jpg'
    const timestamp = Date.now()
    const random = Math.random().toString(36).substring(2, 9)
    const filePath = `${props.processId}-${timestamp}-${random}/${timestamp}-${random}.${ext}`
...
const handleSubmit = async () => {
  ...
  try {
    ...
    for (let i = 0; i < validRows.length; i++) {
      ...
      payload.push({
        dewatering_id: props.processId,
        material_id: row.material_id,
        container_number: row.container_number,
        qty: parseFloat(row.qty),
        container_content: row.container_content || null,
    ...
    await dewateringStore.updateProcessInputAmount(props.processId, totalInput)
